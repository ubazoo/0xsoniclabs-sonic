// Code generated by "stringer -type=Feature"; DO NOT EDIT.

package opera

import "strconv"

func _() {
	// An "invalid array index" compiler error signifies that the constant values have changed.
	// Re-run the stringer command to generate them again.
	var x [1]struct{}
	_ = x[SonicCertificateChain-0]
	_ = x[NetworkRuleChecks-1]
	_ = x[EIP7702_SetEoaCode-2]
}

const _Feature_name = "SonicCertificateChainNetworkRuleChecksEIP7702_SetEoaCode"

var _Feature_index = [...]uint8{0, 21, 38, 56}

func (i Feature) String() string {
	if i < 0 || i >= Feature(len(_Feature_index)-1) {
		return "Feature(" + strconv.FormatInt(int64(i), 10) + ")"
	}
	return _Feature_name[_Feature_index[i]:_Feature_index[i+1]]
}
