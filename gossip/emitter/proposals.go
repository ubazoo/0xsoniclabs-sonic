...
func (em *Emitter) addProposal(
	...
	if parents := event.Parents(); len(parents) == 0 {
		...
		for _, parent := range parents {
			//fmt.Printf("\tParent %v\n", parent)
			payload := em.world.GetEventPayload(parent).Payload()
			turn := payload.LastSeenProposalTurn
			block := payload.LastSeenProposedBlock
			frame := payload.LastSeenProposalFrame
...
func (em *Emitter) getTransactionsForProposal(
	...
	fmt.Printf("Scheduling took %v, %d runs, %.1f%% full\n", time.Since(start), numEvaluations, 100*float64(bestGasUsed)/float64(gasLimit))
