...
// implementation with the real-world implementations of the evaluator and
// scrambler. With specifics are tested in individual unit tests, the focus
// of these tests it to verify the overall integration of the components.

func TestIntegration_NoTransactions_RunsNoEvaluationsAndProducesAnEmptySchedule(t *testing.T) {
	scheduler := NewScheduler(nil)
	require.Empty(t, scheduler.Schedule(
		t.Context(),
		&BlockInfo{},
		slices.Values([]*types.Transaction{}),
		100_000_000,
	))
}
...
func TestIntegration_OneTransactions_ProducesScheduleWithOneTransaction(t *testing.T) {
	...
	// execution, hence, it is not skipped.
	any := gomock.Any()
	...
	schedule := NewScheduler(world).Schedule(
		...
		slices.Values([]*types.Transaction{tx}),
		100_000_000,
	)

	require.Len(t, schedule, 1)
	require.Equal(t, tx, schedule[0])
}
