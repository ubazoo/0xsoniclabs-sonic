...
library BigNumbers {
    ...
    function add(
        ...
        if(a.neg || b.neg){
            if(a.neg && b.neg){
                ...
                if(compare==1){
                    (val, bitlen) = subPrivate(a.val,b.val);
                    r.neg = a.neg;
                }
                else if(compare==-1){
                    (val, bitlen) = subPrivate(b.val,a.val);
                    r.neg = !a.neg;
                }
    ...
    function mul(
        ...
        if(!eq(a,b)) {
            ...
            r = shrPrivate(sub(fst, snd) , 2); // (a * b) = (((a + b)**2 - (a - b)**2) / 4
        }
        else {
            r = shrPrivate(fst, 2); // a==b ? (((a + b)**2 / 4
        }
    }
    ...
    /** @notice right shift BigNumber value
      * @dev shr: right shift BigNumber a by 'bits' bits.
             copies input value to new memory location before shift and calls shrPrivate function after. 
    ...
    function shr(
        ...
        require(!a.neg);
        return shrPrivate(a, bits);
    }

    /** @notice right shift BigNumber memory 'dividend' by 'bits' bits.
      * @dev shrPrivate: Shifts input value in-place, ie. does not create new memory. shr function does this.
      ...
      * @return r result
      */
    function shrPrivate(BigNumber memory bn, uint bits) public view returns(BigNumber memory){
        uint length;
    ...
    /** @notice takes two BigNumber memory values and subtracts them.
      * @dev subPrivate: This function is private and only callable from add: therefore the values may be of different sizes, 
      *            in any order of size, and of different signs (handled in add).
